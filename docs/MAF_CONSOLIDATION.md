...
| Precompiled release automation | Replaced with one matrix that builds the CLI and GUI for Linux, Windows, Intel macOS, and Apple Silicon macOS, plus checksums, a native Windows installer, a macOS app bundle, and platform setup guides. |
...
| MAF branding/assets and old installer metadata | Removed. Release archives are the supported precompiled distribution and carry Sensitivity branding and AGPL metadata. |
